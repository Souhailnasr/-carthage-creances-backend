...
public class UtilisateurController {
     ...
     *
     * @param userId L'ID de l'utilisateur
     * @param request Body contenant le nouveau mot de passe
     * @return ResponseEntity avec l'utilisateur mis à jour (200 OK) ou erreur
     *
     * @example
    ...
    public ResponseEntity<?> reinitialiserMotDePasse(
        ...
        try {
            ...
            Utilisateur updatedUser = utilisateurService.reinitialiserMotDePasse(userId, nouveauMotDePasse);
            return ResponseEntity.ok(Map.of(
            ...
            logger.error("Erreur lors de la réinitialisation du mot de passe pour l'utilisateur " + userId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
     ...
     * GET /api/users/chef-dossier/email/{email}
     *
     * @param email L'email du chef dossier
     * @return ResponseEntity avec le chef dossier (200 OK) ou erreur (404 NOT_FOUND)
     *
     * @example
     * GET /api/users/chef-dossier/email/chef.dossier@example.com
     */
    @GetMapping("/chef-dossier/email/{email}")
    public ResponseEntity<?> getChefDossierByEmail(@PathVariable String email) {
        try {
            Optional<Utilisateur> utilisateur = utilisateurService.getUtilisateurByEmail(email);
            if (utilisateur.isPresent()) {
                Utilisateur user = utilisateur.get();
                if (user.getRoleUtilisateur() == RoleUtilisateur.CHEF_DEPARTEMENT_DOSSIER) {
                    return ResponseEntity.ok(Map.of(
                            "id", user.getId(),
                            "nom", user.getNom(),
                            "prenom", user.getPrenom(),
                            "email", user.getEmail(),
                            "roleUtilisateur", user.getRoleUtilisateur().name()
                    ));
                } else {
                    return ResponseEntity.badRequest()
                            .body(Map.of("error", "L'utilisateur avec cet email n'est pas un chef dossier"));
                }
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(Map.of("error", "Chef dossier non trouvé avec cet email"));
            }
        } catch (Exception e) {
            logger.error("Erreur lors de la recherche du chef dossier par email: " + email, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
     ...
     * GET /api/users/chef-dossier/all
     *
     * @return ResponseEntity avec la liste des chefs dossier (200 OK)
     *
     * @example
     * GET /api/users/chef-dossier/all
     */
    @GetMapping("/chef-dossier/all")
    public ResponseEntity<?> getAllChefDossier() {
        try {
            List<Utilisateur> chefs = utilisateurService.getUtilisateursByRoleUtilisateur(RoleUtilisateur.CHEF_DEPARTEMENT_DOSSIER);
            return ResponseEntity.ok(chefs.stream().map(chef -> Map.of(
                    "id", chef.getId(),
                    "nom", chef.getNom(),
                    "prenom", chef.getPrenom(),
                    "email", chef.getEmail(),
                    "roleUtilisateur", chef.getRoleUtilisateur().name()
            )).collect(java.util.stream.Collectors.toList()));
        } catch (Exception e) {
            logger.error("Erreur lors de la récupération des chefs dossier", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
